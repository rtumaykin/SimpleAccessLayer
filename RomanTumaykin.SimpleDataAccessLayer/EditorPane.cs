...
namespace RomanTumaykin.SimpleDataAccessLayer
{
	...
	//                                IVsToolboxUser      //Sends notification about Toolbox items to the owner of these items
	{
		...
		private bool fileFormatValid = true;
		...
		#region Fields
		private SimpleDataAccessLayerConfigFileEditorPackage myPackage;
		...
		{
			pClassID = GuidList.guidSimpleDataAccessLayerConfigFileEditorEditorFactory;
		...
		int IPersistFileFormat.Load(string pszFilename, uint grfMode, int fReadOnly)
		{
			...
			try
			{
				...
				DalConfig _config = null;
				try
				{
					DataContractSerializer _ser = new DataContractSerializer(typeof(DalConfig));
					_config = (DalConfig)_ser.ReadObject(XmlReader.Create(pszFilename));
				}
				catch
				{
					_config = new DalConfig()
					{
						...
						Enums = new System.Collections.Generic.List<Enum>(),
						Namespace = "",
						Procedures = new System.Collections.Generic.List<Procedure>()
					};
				}
				finally
				{
					editorControl.Init(_config, pszFilename, myPackage);
				}
		...
		int IPersistFileFormat.Save(string pszFilename, int fRemember, uint nFormatIndex)
		{
			...
			try
			{
				editorControl.SaveConfig(pszFilename);
				// implement a different way of doing this editorControl.RichTextBoxControl.SaveFile(pszFilename, RichTextBoxStreamType.RichText);
			}
